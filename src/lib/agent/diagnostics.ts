...
/** What the completed tool steps tell us about why the turn ended empty. */
interface StepOutcomes {
	successfulWrite: SearchStep | undefined;
	anyToolCompleted: boolean;
	emailListed: boolean;
	emailSummarized: boolean;
	imageSearched: boolean;
	webResearched: boolean;
}

function summarizeStepOutcomes(searchSteps: SearchStep[]): StepOutcomes {
	...
	return {
		successfulWrite,
		anyToolCompleted: doneSteps.length > 0,
		emailListed: doneSteps.some((s) => s.toolName === 'email_list_recent'),
		emailSummarized: doneSteps.some((s) => s.toolName === 'email_summarize_message'),
		imageSearched: doneSteps.some(
			(s) => s.toolName === 'image_search' || s.toolName === 'fetch_url_images'
		),
		webResearched: doneSteps.some(
			(s) =>
...
/** Map the step outcomes to a commit/error diagnosis, in priority order. */
function classifyEmptyResponse(o: StepOutcomes): Diagnosis {
	if (o.successfulWrite) {
		return { type: 'commit', content: `Done. File written: ${o.successfulWrite.query}` };
	}
	if (o.emailListed && !o.emailSummarized) {
		return {
			type: 'error',
			message:
...
export function diagnoseEmptyResponse(
	...
	logDebug('diagnose', 'diagnoseEmptyResponse called', {
		...
		emailSummarized: outcomes.emailSummarized,
		imageSearched: outcomes.imageSearched,
		webResearched: outcomes.webResearched
	});

	return classifyEmptyResponse(outcomes);
}
