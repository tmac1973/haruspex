...
fn model_registry() -> Vec<ModelInfo> {
    vec![
        ModelInfo {
            ...
            sha256: String::new(), // Populated on first verified download
            size_bytes: 1_890_000_000,
            description: "Smallest — for very constrained RAM/VRAM (~1.9 GB)".to_string(),
            downloaded: false,
        },
        ModelInfo {
            ...
            description: "Recommended — best quality/size balance (~2.1 GB)".to_string(),
            downloaded: false,
        },
        ModelInfo {
            ...
            description: "Higher quality — for 6+ GB VRAM (~2.4 GB)".to_string(),
            downloaded: false,
        },
        ModelInfo {
            id: "granite-4.0-micro-Q8_0".to_string(),
            filename: "granite-4.0-micro-Q8_0.gguf".to_string(),
            url: "https://huggingface.co/unsloth/granite-4.0-micro-GGUF/resolve/main/granite-4.0-micro-Q8_0.gguf".to_string(),
            sha256: String::new(),
            size_bytes: 3_620_000_000,
            description: "Highest quality — for 6+ GB VRAM (~3.6 GB)".to_string(),
            downloaded: false,
        },
    ]
}
...
mod tests {
    ...
    fn model_registry_has_expected_entries() {
        ...
        assert!(ids.contains(&"granite-4.0-micro-Q8_0"));
    }
    ...
    fn recommended_quant_based_on_ram() {
        // The function uses available_ram_mb from sysinfo, but we can verify the logic:
        // < 4096 MB → IQ4_XS
        // 4096-6144 → Q4_K_M
        // > 6144 → Q5_K_M
        let info = detect_hardware();
